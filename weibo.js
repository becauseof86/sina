function SSOController() {
    var undefined, me = this,
        ...
        crossDomainTimer = null,
        crossDomainTime = 3,
        autoLoginCallBack2 = null,
        ssoCrosssDomainUrl = "http://login.sina.com.cn/sso/crossdomain.php",
        ssoLoginUrl = "http://login.sina.com.cn/sso/login.php",
        ssoLogoutUrl = "http://login.sina.com.cn/sso/logout.php",
        ssoUpdateCookieUrl = "http://login.sina.com.cn/sso/updatetgt.php",
        ssoPreLoginUrl = "https://login.sina.com.cn/sso/prelogin.php",
        pincodeUrl = "http://login.sina.com.cn/cgi/pin.php",
        vfValidUrl = "http://weibo.com/sguide/vdun.php",
        generateVisitorUrl = "http://passport.weibo.com/visitor/visitor",
        crossDomainUrlList = null,
        loginMethod = "",
        ssoServerTimeTimer = null,
        ssoLoginTimer = null,
        loginByConfig = null,
            ...
            if (d) {
                ...
                if (me.feedBackUrl) me.crossDomain ? me.crossDomainAction("login", function () {
                    ...
                    me.crossDomainAction("login", function () {
                        if (c && me.appLoginURL[me.domain]) me.appLogin(c, me.domain, me.name + ".callbackLoginStatus");
                        else {
    ...
    var makeNonce = function (a) {
        ...
        loginByXMLHttpRequest = function (a, b, c) {
            ...
            if (!1 in d) return !1;
            var e = makeXMLRequestQuery(a, b, c),
                f = me.loginType & https ? ssoLoginUrl.replace(/^http:/, "https:") : ssoLoginUrl;
            f = makeURL(f, {
                client: me.getClientType(),
                _: (new Date).getTime()
            });
        ...
        loginByIframe = function (a, b, c) {
            ...
            var d = createForm(me.loginFormId),
                e = makeRequest(a, b, c);
            e.encoding = "UTF-8";
            me.crossDomain == !1 && (e.crossdomain = 0);
            e.prelt = preloginTime;
            if (me.feedBackUrl) {
                e.url = makeURL(me.feedBackUrl, {
                    framelogin: 1,
...
function get_su(name){
    return sinaSSOEncoder.base64.encode(name);
}
function get_sp(pwd,nonce,servertime,rsaPubkey){
    var e = new sinaSSOEncoder.RSAKey();
    e.setPublic(rsaPubkey, "10001");
    sp = e.encrypt([servertime,nonce].join("\t") + "\n" + pwd);
    return sp;
}
//console.log(get_su('15252461911'));
//console.log(get_sp('Dwj882193','RS17UP','1498832106',''));
